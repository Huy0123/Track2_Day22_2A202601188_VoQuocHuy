...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            return FailResult(
                error_message="JSON đã được sửa lỗi",
                fix_value=formatted,
            )
...
def demo_pii_guard():
    ...
    (evidence_dir / "04_pii_demo_log.txt").write_text(output_str, encoding="utf-8")
...
def demo_json_guard():
    ...
    evidence_dir = Path(__file__).parent.parent / "evidence"
    evidence_dir.mkdir(parents=True, exist_ok=True)
    (evidence_dir / "04_json_demo_log.txt").write_text(output_str, encoding="utf-8")
